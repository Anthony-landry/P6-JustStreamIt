...
async function loadBestFilm() {
	...
	let page = "";
	let best = "";
	let lists_films_array = [];
	let list_films = [];

	for (let i = 0; i < 2; i++) {
		page = pageArray[i];
		best = "titles/?" + page + "sort_by=-imdb_score";
		let reponse = await fetch(path + best);
		if (reponse.ok) {
...
async function loadCategorie(categorie) {
	...
	for (let i = 0; i < 2; i++) {
		page = pageArray[i];
		categorieAPI =
			"titles/?" + page + "sort_by=-imdb_score&genre_contains=" + categorie;
		let reponse = await fetch(path + categorieAPI);
		if (reponse.ok) {
			let films = await reponse.json();
			// Transformation des films en JSON
			list_films = films.results;
...
async function loadSessionStorage() {
	...
	if (best_films === null || best_films == "{}") {
		let result_load = await loadBestFilm().catch((err) => {
	...
	if (filmsAdventure === null || filmsAdventure == "{}") {
		let result_load2 = await loadCategorie("Adventure").catch((err) => {
			alert("Le serveur à repondu :" + err);
		});
		if (result_load2 == 200) {
			categorie1OK = 1;
	...
	if (filmsComedy === null || filmsComedy == "{}") {
		let result_load3 = await loadCategorie("Comedy").catch((err) => {
			alert("Le serveur à repondu :" + err);
		});
		if (result_load3 == 200) {
			categorie2OK = 1;
	...
	if (filmsFantasy === null || filmsFantasy == "{}") {
		let result_load4 = await loadCategorie("Fantasy").catch((err) => {
			alert("Le serveur à repondu :" + err);
		});
		if (result_load4 == 200) {
			categorie3OK = 1;
	...
	if (
		...
		alert("erreur chargement du local storage");
		return 0;
	}
}
...
async function best_film(sessionStorageOK) {
	...
	if (sessionStorageOK) {
		...
		let best_films = window.sessionStorage.getItem(categorie);
		...
		const zoneTitle = document.querySelector(".zone-button p");
		zoneTitle.innerText = bestFilm.title;
		section1.appendChild(imageFilm);

		// Création du bouton Film
		const zoneBouton = document.querySelector(".zone-button button");
		// Création du modale
...
async function modale(filmUrl, zoneModale) {
	...
	try {
		film_string = await loadFilm(filmUrl);
	} catch (e) {
		console.error(e.name); // affiche 'Error'
		console.error(e.message);
	}
	let film = JSON.parse(film_string);
	if (!document.getElementById("caption" + film.title)) {
		...
		filmBoxOffice.innerText =
			"Résultat au box office : " + (film.metascore ?? "pas de résultat.");
		const filmDescription = document.createElement("p");
		filmDescription.innerText =
			"Résumé : " + (film.long_description ?? film.description);
...
async function sevenBestFilm(sessionStorageOK) {
	...
	if (sessionStorageOK) {
		...
		for (let i = 1; i < 8; i++) {
			let film = best_films_array[i];
			// Récupération de l'élément du DOM qui accueillera les films
			let section = document.querySelector("#categorie > div > div.slide");
			// Création de l'image + titre du film
...
async function categorie(sessionStorageOK, categorie, nb) {
	...
	if (sessionStorageOK) {
		...
		for (let i = 0, y = 0; i < nbmaxFilm && y < 7; i++) {
			let film = best_films_array[i];
			// Récupération de l'élément du DOM qui accueillera les films
			let section = document.querySelector(
				"#" + categorie + " > div > div.slide"
			);
			// Création de l'image + titre du film
			...
			try {
				const reponse = await fetch(
					film.image_url
				); /* on part sur le fait que CORS soit une erreur par le browser, impossible de récuperer le status 404 */
				...
				window.sessionStorage.setItem(
					categorie,
					JSON.stringify(best_films_array)
				);
				i -= 1;
				continue;
			}
...
function slider(categorie) {
	...
	prevBtn.addEventListener("click", function () {
		slider.style.marginLeft =
			parseInt(slider.style.marginLeft || 0) + 100 + "px";
...
async function main() {
	let sessionStorageOK = await loadSessionStorage();
